...
namespace NerdDinner.Models
{
    public class NerdDinners : DbContext
    {
        ...
        public IQueryable<Dinner> FindByLocation(float latitude, float longitude)
        {
            ...
            string sqlQuery = "CREATE FUNCTION [dbo].[NearestDinners] (@lat real, @long real) RETURNS TABLE AS RETURN SELECT Dinners.DinnerID FROM   Dinners WHERE " + firstSqlQuery + " < 100";
            Dinner nearest = Database.SqlQuery<Dinner>(sqlQuery).FirstOrDefault();
            var dinners = from dinner in FindUpcomingDinners()
                join i in Database.SqlQuery<Dinner>(sqlQuery)
